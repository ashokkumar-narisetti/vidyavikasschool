...
export default function AdminAchievements() {
    ...
    const uploadPhoto = async (file) => {
        ...
        const { error: err } = await supabase.storage.from('school-assets').upload(path, file, { upsert: true });
        if (err) throw err;
    ...
    const handleSave = async (e) => {
        e.preventDefault();
        if (!form.student_name || !form.title || !form.year) {
        ...
        if (form.photoFile) {
            try { imageUrl = await uploadPhoto(form.photoFile); }
            catch (err) { setError('Photo upload failed.'); setSaving(false); return; }
        }
        const payload = {
                    ...
                    <form onSubmit={handleSave} className="admin-achievement-form">
                        <div className="admin-form-row">
                            <div className="admin-form-group">
                                ...
                                <input className="admin-input" placeholder="Full Name" value={form.student_name}
                                    onChange={e => setForm(f => ({ ...f, student_name: e.target.value }))} required />
                            </div>
                            <div className="admin-form-group">
                                <label>Title *</label>
                                <input className="admin-input" placeholder="e.g. School Topper, Olympiad Winner" value={form.title}
                        ...
                        <div className="admin-form-row">
                            <div className="admin-form-group">
                                <label>Year *</label>
                                <input className="admin-input" placeholder="e.g. 2023-24" value={form.year}
                            ...
                            <div className="admin-form-group">
                                ...
                                <label className="admin-file-label">
                                    ...
                                    <input type="file" accept="image/*" style={{ display: 'none' }}
                                        onChange={e => setForm(f => ({ ...f, photoFile: e.target.files[0] }))} />
                                </label>
                            </div>
                        </div>
                        <div className="admin-form-group">
                            <label>Description</label>
                            <textarea className="admin-input" rows={3} placeholder="Achievement details..."
                        ...
                        {achievements.map(ach => (
                            <div key={ach.id} className="admin-ach-card">
                                {ach.image_url ? (
                                    ...
                                    <div className="admin-ach-photo-placeholder">🎓</div>
                                )}
